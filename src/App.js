...
const GoalAchievementWorkbook = () => {
  ...
  const round1Steps = [
    { id: 0, title: '기본 정보 입력', subtitle: '작성일, 지원 직무, 회사명, 경험 제목, 경험 시기, 소요 기간, 관련 직무 역량을 입력하세요' },
    {
      id: 1,
  ...
  const generateFinalText = () => {
    ...
    text += `작성일: ${basicInfo.writeDate}\n`;
    text += `지원 직무: ${basicInfo.position}\n`;
    text += `회사명: ${basicInfo.company}\n`;
    text += `경험 제목: ${basicInfo.experienceTitle}\n`;
    text += `경험 시기: ${basicInfo.experiencePeriod}\n`;
    text += `소요 기간: ${basicInfo.duration}\n`;
    text += `관련 직무 역량: ${basicInfo.relatedCompetency}\n\n`;
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleBasicInfoChange('writeDate', e.target.value)}
