      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-2xl p-8">
            <h1 className="text-4xl font-bold text-gray-800 mb-4 text-center">질문에 답하며 완성하는<br />목표수립 및 달성 워크북</h1>
            <p className="text-center text-gray-600 mb-8">CareerEngineer의 3라운드 체계적 작성 시스템</p>

            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6">
              <h2 className="text-xl font-bold text-gray-800 mb-3">이 워크북이 완성하는 스토리 흐름</h2>
              <div className="space-y-2">
                {[
                  ['Q1', '목표 정의', '목표가 제대로 정의되어 있는가? 기대효과와 도전의 이유'],
                  ['Q2', '계획 수립', '어떤 계획을, 왜 그 방식으로 세웠는가?'],
                  ['Q3', '실행과 극복', '계획과 달랐던 것들을 어떻게 극복했는가?'],
                  ['Q4', '결과와 임팩트', '달성 결과·목적 달성 여부·기대효과 충족·임팩트'],
                  ['Q5', '노력 과정', '과정에서의 노력이 제대로 드러나는가?'],
                  ['Q6', '배움과 기여', '배운 것이 직무와 연결되고 기여로 이어지는가?'],
                ].map(([q, title, desc]) => (
                  <div key={q} className="flex items-start gap-3 bg-white rounded p-3">
                    <span className="text-blue-600 font-bold text-base w-6 shrink-0">{q}</span>
                    <div><span className="font-bold text-gray-800">{title}</span><span className="text-gray-500 ml-2 text-sm">{desc}</span></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                <div key={title} className={`bg-white rounded-lg p-4 border-l-4 ${border}`}>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-6 mb-6">
              ...
              <ul className="space-y-1 text-sm text-gray-700">
                ...
                <li><strong>연결성:</strong> 목표 정의 → 계획 → 극복 → 결과 → 임팩트 → 배움 → 기여가 인과 흐름으로 이어져야 합니다</li>
                <li><strong>비교 가능성:</strong> "기대했던 것"과 "실제 된 것"의 비교가 스토리에 깊이를 만듭니다</li>
              </ul>
              <div className="mt-4 pt-4 border-t border-yellow-300">
                <p className="text-sm font-semibold text-gray-800 mb-1">💡 3초 자가진단이란?</p>
            ...
            <div className="bg-red-50 border-2 border-red-300 rounded-lg p-6 mb-6">
              ...
              <p className="text-sm text-red-700">작성하는 내용은 자동으로 저장되지 않으며 새로고침 버튼을 누르면 그동안 작성했던 내용은 사라집니다. 마지막 페이지에서 반드시 워드 파일(.doc)로 다운로드 하여 보관하세요.</p>
            </div>

            <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
              <div className="mt-4 pt-4 border-t border-gray-200">
                <p className="text-xs text-gray-800 text-center">© 2026 CareerEngineer All Rights Reserved.</p>
                <p className="text-xs text-red-800 text-center mt-1 font-semibold">이 워크북은 저작권법에 의해 보호받는 저작물입니다. 무단 복제·배포·전송·수정을 금하며, 오직 개인적인 용도로만 사용해야 합니다.</p>
              </div>
            </div>

            <button onClick={() => setShowIntro(false)} className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-colors font-bold text-lg">1라운드 시작하기 →</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <p className="text-center text-gray-600 mb-4">답변이 얕거나 더 구체화가 필요한 질문을 선택하여 2라운드에서 심화 질문에 답변하세요</p>
            <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-6 text-sm text-amber-900 rounded">
              ...
              <p>답변을 다시 읽었을 때 면접관이 <strong>"더 구체적으로 말해줄 수 있어요?"</strong>라고 물을 것 같은 질문을 선택하세요.</p>
              <p className="mt-1 text-amber-700">특히 3초 자가진단을 통과하기 어려웠던 질문을 우선 선택하세요.</p>
            </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="bg-gray-50 rounded p-3 text-sm text-gray-700">
                          <strong>내 답변:</strong> {answers[step.questions[0].id]?.substring(0, 100) || '(답변 없음)'}{answers[step.questions[0].id]?.length > 100 && '...'}
                        </div>
                      </div>
                      <button onClick={() => toggleStepSelection(step.id)} className={`ml-4 px-4 py-2 rounded-lg font-semibold transition-colors ${isSelected ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
            ...
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
              <p className="text-sm text-blue-800"><strong>💡 선택 기준:</strong> 답변이 부족하거나 더 구체화가 필요한 질문을 자유롭게 선택하세요. (1개 이상)</p>
            </div>
            <div className="flex gap-4">
              <button onClick={goToPrevStep} className="flex items-center gap-2 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium"><ChevronLeft className="w-5 h-5" />이전</button>
              <button onClick={goToNextStep} disabled={!canGoNext()} className="flex-1 py-4 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-lg">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-4"><Check className="w-10 h-10 text-green-600" /></div>
              <h2 className="text-3xl font-bold text-gray-800 mb-2">목표수립 및 달성 완성! 🎉</h2>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg p-5 mb-6">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Edit3 className="w-5 h-5 text-blue-600" />완성된 목표수립 및 달성 (수정 가능)</h3>
                <button onClick={() => setShowRawAnswers(!showRawAnswers)} className="text-sm text-gray-600 hover:text-gray-800 flex items-center gap-1"><Eye className="w-4 h-4" />{showRawAnswers ? '원본 답변 숨기기' : '원본 답변 보기'}</button>
              </div>
              ...
              <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-4 rounded">
                ...
                <p className="text-sm text-amber-800 mb-1">구조: <strong>[목표·도전의 배경]</strong>에서 시작 → <strong>[계획과 극복]</strong> 연결 → <strong>[결과·임팩트]</strong> → <strong>[배움·역량·기여]</strong> 마무리</p>
                <p className="text-xs text-amber-700 mt-2 border-t border-amber-200 pt-2">⚠️ 피해야 할 표현: "열심히 했습니다" / "결과가 좋았습니다" — 모두 구체성이 없습니다</p>
              </div>
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
                ...
                {[
                  ...
                  <div key={sec.title} className={`bg-white border-l-4 ${sec.color === 'purple' ? 'border-purple-500' : 'border-pink-500'} rounded p-3 mb-3`}>
                    <p className={`text-xs font-bold ${sec.color === 'purple' ? 'text-purple-700' : 'text-pink-700'} mb-2`}>{sec.title}</p>
                    <p className="text-xs text-gray-500 mb-1">👉 {sec.hint}</p>
                    {answers[sec.connId] && (
                      <div className={`${sec.color === 'purple' ? 'bg-purple-50' : 'bg-pink-50'} rounded p-2 mb-2`}>
                        <p className={`text-xs ${sec.color === 'purple' ? 'text-purple-600' : 'text-pink-600'} font-semibold`}>✅ {sec.connLabel}</p>
                        <p className="text-xs text-gray-700 mt-1 whitespace-pre-wrap">{answers[sec.connId].substring(0,150)}{answers[sec.connId].length>150?'...':''}</p>
                      </div>
                    )}
                    {sec.ids.map(([id, label]) => answers[id] && (
                      <div key={id} className="bg-gray-50 rounded p-2 mb-1">
                        <p className="text-xs text-gray-500 font-semibold">{label}</p>
                        <p className="text-xs text-gray-700 mt-1">{answers[id].substring(0,100)}{answers[id].length>100?'...':''}</p>
                      </div>
                    ))}
                    <p className={`text-xs ${sec.color === 'purple' ? 'text-purple-600' : 'text-pink-600'} mt-2 italic`}>{sec.tip}</p>
                  </div>
                ))}
              </div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="bg-white rounded p-3 border border-blue-100">
                      <p className="text-xs font-semibold text-blue-800 mb-1">{`${['①','②','③','④','⑤'][i]} ${check}`}</p>
                      <p className="text-xs text-gray-500">통과 못 하면 → <span className="text-purple-600 font-semibold">{ref}</span>를 다시 확인하세요</p>
                      {ansId && answers[ansId] && <p className="text-xs text-gray-600 mt-1 bg-gray-50 rounded p-1 italic">"{answers[ansId].substring(0,60)}{answers[ansId].length>60?'...':''}"</p>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {downloadSuccess && (
              <div className="bg-green-100 border-2 border-green-500 rounded-lg p-4 text-center mb-4">
                ...
                <p className="text-sm text-green-700 mt-1">다운로드 폴더에서 "{basicInfo.company || '회사'}_목표수립및달성.doc" 파일을 Microsoft Word로 열어주세요.</p>
              </div>
            )}
            ...
            <div className="flex gap-4 mt-4">
              <button onClick={goToPrevStep} className="flex items-center gap-2 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium"><ChevronLeft className="w-5 h-5" />이전으로</button>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">CareerEngineer 목표수립 및 달성 워크북</h1>
          <p className="text-gray-600">체계적인 3라운드 시스템으로 완성하는 목표달성 스토리</p>
          <div className="mt-4">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
              ...
              {currentStepData.questions.map(q => (
                <div key={q.id} className="mb-6 border-b border-gray-200 pb-6 last:border-b-0">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {q.guide && (
                      <button onClick={() => toggleGuide(q.id)} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 shrink-0 ml-4">
                  ...
                  {q.referenceQuestions && (
                    <div className={`border-l-4 p-4 mb-4 rounded-r-lg ${currentPhase === 'round3' ? 'bg-purple-50 border-purple-400' : 'bg-indigo-50 border-indigo-400'}`}>
                      ...
                      {currentPhase === 'round3' && <p className="text-xs text-purple-700 mb-3">모든 내용을 다 쓸 필요는 없습니다. 각 답변에서 가장 핵심적인 부분을 골라 자연스럽게 연결하세요.</p>}
                      <div className="space-y-3">
                        {q.referenceQuestions.map(refId => {
                          const allQs = round1Steps.flatMap(s => s.questions || []);
                          const refQ = allQs.find(q => q?.id === refId);
                          ...
                          return (
                            <div key={refId} className={`rounded text-sm p-3 ${currentPhase === 'round3' ? 'bg-white border border-purple-100' : 'bg-white'}`}>
                              <p className={`font-semibold mb-1 text-xs ${currentPhase === 'round3' ? 'text-purple-700' : 'text-gray-700'}`}>{refQ.label}</p>
                              <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">{answers[refId]?.substring(0, limit)}{answers[refId]?.length > limit ? '...' : ''}</p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-3 space-y-3">
                      ...
                      {q.guide.helpQuestions && (
                        <div>
                          ...
                          <ul className="text-sm text-blue-800 space-y-1 ml-4">{q.guide.helpQuestions.map((hq, i) => <li key={i}>• {hq}</li>)}</ul>
                        </div>
                      )}
                      {q.guide.ifDifficult && <div><p className="text-sm font-semibold text-blue-900 mb-1">💭 답변하기 어렵다면:</p><p className="text-sm text-blue-800">{q.guide.ifDifficult}</p></div>}
                      {q.guide.ifStillDifficult && <div><p className="text-sm font-semibold text-blue-900 mb-1">💡 구체화 도움 질문으로도 어렵다면:</p><p className="text-sm text-blue-800">{q.guide.ifStillDifficult}</p></div>}
                    </div>
                  )}
          ...
          <div className="flex gap-4 mt-8">
            <button onClick={goToPrevStep} className="flex items-center gap-2 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium"><ChevronLeft className="w-5 h-5" />이전</button>
            <button onClick={goToNextStep} disabled={!canGoNext()} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold">
